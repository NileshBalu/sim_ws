...
public:
  A2Bridge() : Node("a2_bridge_node") {
    ...
    // 3. Define active joint names (excluding fixed foot joints)
    joint_names_ = {
...
private:
  void lowstate_callback(const unitree_go::msg::LowState::SharedPtr msg) {
    ...
    for (size_t i = 0; i < joint_names_.size(); ++i) {
      js_msg.position.push_back(msg->motor_state[i].q);
      js_msg.velocity.push_back(msg->motor_state[i].dq);
      js_msg.effort.push_back(msg->motor_state[i].tau_est);
    }
